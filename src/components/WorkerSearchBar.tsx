  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (cosaTrimmed) {
        ...
        filtered = filtered.filter(worker => normalizeString(worker.job_title).includes(normalizedCosa));
      }
      if (doveTrimmed) {
        const normalizedDove = normalizeString(doveTrimmed);
        filtered = filtered.filter(worker => normalizeString(worker.location).includes(normalizedDove));
      }
      ...
      <div className="container mx-auto bg-white rounded-2xl shadow-lg p-6 mb-8">
          <div className="grid md:grid-cols-[1fr,1fr,auto] gap-4 items-end">
            <div>
              ...
              <Input id="cosa-search" type="text" placeholder="Cosa cerchi (es. Magazziniere, Operaio)" value={cosa} onChange={e => setCosa(e.target.value)} className="w-full" aria-label="Cerca per mansione" />
            </div>
        ...
        {hasSearched && <div className="container mx-auto">
            {isLoading ? <div className="text-center py-8">
                ...
                <div className="grid gap-4 mb-6">
                  {results.map((worker, index) => <div key={index} onClick={handleCardClick} className="bg-white rounded-xl shadow-md p-6 hover:shadow-xl transition-all cursor-pointer border border-gray-100 hover:border-airstaff-pink">
                ...
                <p className="text-gray-700 text-lg">
                  ...
                  Nessun problema,{' '}
                  <a href="https://airstaff.it/apri-posizione" target="_self" className="text-airstaff-pink hover:text-airstaff-blue font-semibold transition-colors">
